...
function autocomplete(input, tagArray) {
    ...
    input.addEventListener("input", function(e) {
        ...
        for (i = 0; i < tagArray.length; i++) {
          ...
          if (tagArray[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
            /*create a DIV element for each matching element:*/
            b = document.createElement("DIV");
            b.innerHTML = "<strong>" + tagArray[i].substr(0, val.length) + "</strong>";
    ...
    function closeAllLists(element) {
      var x = document.getElementsByClassName("add-offer-tags-autocomplete__items");
      for (var i = 0; i < x.length; i++) {
